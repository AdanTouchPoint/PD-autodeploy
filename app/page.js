...
async function Home() {
  ...
  const id = process.env.NEXT_PUBLIC_CAMPAING_ID || "686ee08f96979d30fba21c2b";
  ...
  const endpoints = {
    ...
    toGetMainData: "/campaignContentId",
    toSaveLeads: "/leads/",
  };

  let initialState;
  try {
    initialState = await fetchMainContent(backendURLBase, id, clientId, campaignType);
  } catch (error) {
    console.error(error);
